...
namespace CMS.Web.HttpModules
{
    public sealed class CookieHelper
    {
        ...
        internal void SetLoginCookie(string userId, string password, bool isPermanentCookie, int timeExpireLogin)
        {
            if (_response != null)
            {
                if (isPermanentCookie)
                {
                    if (timeExpireLogin == 0) timeExpireLogin = 30; // default time login
                    var dateExpire = DateTime.Now.AddMinutes(timeExpireLogin);
                    FormsAuthenticationTicket userAuthTicket =
                        new FormsAuthenticationTicket(1, userId, DateTime.Now,
                            dateExpire, true, password, FormsAuthentication.FormsCookiePath);
                    string encUserAuthTicket = FormsAuthentication.Encrypt(userAuthTicket);
        ...
        public bool AuthenticateUser(string username, string password, bool persistLogin)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    ...
                    newCookieHelper.SetLoginCookie(user.Id.ToString(), password, true, user.TimeExpireLogin);
